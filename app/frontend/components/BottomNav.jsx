...
export default function BottomNav({ active }) {
  const unread = usePage().props.chat_unread || 0
  const cls = (k) => `n ${active === k ? 'on' : ''}`
  return (
    <nav className="nav">
      <Link href="/" className={cls('hub')}><span className="ic">🏠</span>Hub</Link>
      <Link href="/chat" className={cls('chat')}>
        <span className="ic nav-ic">💬{unread > 0 && <span className="nav-badge">{unread > 9 ? '9+' : unread}</span>}</span>Chat
      </Link>
