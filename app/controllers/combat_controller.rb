class CombatController < ApplicationController
  ...
  def props(m)
    foe = m.team.opponent
    {
      ...
      my_team:  { name: m.team.name, monster: monster_json(m.team.monster) },
      foe_team: foe && { name: foe.name, monster: monster_json(foe.monster) },
  ...
  def monster_json(mon)
    ...
    { name: mon.name, hp: mon.hp, max_hp: mon.max_hp, percent: mon.hp_percent,
